...
/**
 \brief La estructura Imagen es la implementación interna del TDA Imagen ofrecido de forma pública por la biblioteca.
 */
struct Imagen
{
    SDL_Surface *surface;
...
/**
  \brief El tipo Color (que es privado al módulo de la biblioteca) permite manejar colores especificados mediante sus componentes RGB y Alpha.
 */
...
/**
   \brief La variable pantalla (que es privada al módulo de la biblioteca) mantiene el estado de la ventana donde se mostrarán todas las primitivas gráficas que se lleven a cabo.
 */
static Pantalla pantalla;

/**
   \brief Función privada para gestión de errores SDL.
 */
void Pantalla_Error()
{
    printf("%s\n", SDL_GetError());
...
/**
   \brief Esta función privada se encarga de capturar el evento de cierre de la ventana
 */
...
void Pantalla_Crea(char *titulo, int w, int h)
{
	...
	// Inicialmente, la ventana será gris, el color de borde negro y el de relleno blanco.
	Pantalla_DibujaRellenoFondo(192,192,192,255);
...
void Pantalla_DibujaCirculo(double x, double y, double r)
{
    ...
    double px = (double)r -0.5;
    double py = (double)0.5;
    double cx = x - 0.5;
    double cy = y - 0.5;

    while (px >= py)
    {
        if (SDL_RenderDrawPoint(pantalla.renderer, (int)(cx + px), (int)(cy + py))==-1) Pantalla_Error();
        if (SDL_RenderDrawPoint(pantalla.renderer, (int)(cx + py), (int)(cy + px))==-1) Pantalla_Error();
...
void Pantalla_DibujaImagen(Imagen imagen, double x, double y, double ancho, double alto)
{
    ...
    dest.w = ancho;
    dest.h = alto;
    if (SDL_BlitSurface(imagen->surface, NULL, pantalla.surface, &dest)==-1) Pantalla_Error();
}
...
Imagen Pantalla_ImagenCaptura(int x1, int y1, int w, int h)
{
    ...
    imagen->surface = SDL_CreateRGBSurface(0, w, h, 32, 0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000);
    if (imagen->surface==NULL) Pantalla_Error();

    if (SDL_BlitSurface(pantalla.surface, &r, imagen->surface, NULL)==-1) Pantalla_Error();
    return imagen;
}
...
int Pantalla_ImagenEscribe(Imagen imagen, char *fichero)
{
    ...
    if (imagen->surface)
    {
        res = SDL_SaveBMP(imagen->surface, fichero);
        return res == 0;
    }
    return 0;
}
